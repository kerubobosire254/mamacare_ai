...
export default function RegisterMother() {
  ...
  const [form, setForm] = useState({
    name: '', phone: '', delivery_date: '', consent_given: false, delivery_type: 'vaginal',
  })
  ...
  async function handleSubmit(e) {
    ...
    setStatus(null)
    try {
      const mother = await api.registerMother({
        name: form.name,
        phone: form.phone,
      ...
      setStatus({ type: 'success', message: `${form.name} registered — Day 3, Day 7, and Week 6 check-ins scheduled.` })
      setTimeout(() => navigate(`/mothers/${mother.id}`), 900)
    } catch (err) {
    ...
    <div>
      ...
      <form className="register-form" onSubmit={handleSubmit}>
        ...
        <div className="field">
          ...
          <input type="date" value={form.delivery_date} onChange={(e) => update('delivery_date', e.target.value)} required />
        </div>
        <div className="field">
        ...
        <div className="field checkbox-field">
          ...
          <label htmlFor="consent" style={{ marginBottom: 0 }}>She has consented to receive automated screening calls</label>
        </div>
